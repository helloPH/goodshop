...
@interface DiscountViewController ()<UITableViewDelegate,UITableViewDataSource,CAAnimationDelegate,MBProgressHUDDelegate,CAAnimationDelegate,newAddresPopDelegate,OrderPromptViewDelegate,UIGestureRecognizerDelegate>
...
- (void)viewDidLoad {
    ...
    discountArray = [NSMutableArray arrayWithCapacity:0];
...
-(void)initNavigation
{
    self.navigationItem.title = @"折扣专区";
...
-(void)initSubviews
{
    ...
    headView.image = [UIImage imageNamed:@"折扣专区"];
...
-(void)getDiscountData
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSMutableDictionary *pram = [NSMutableDictionary dictionaryWithDictionary:@{@"id":_dianpuID,@"pageSize":@"",@"pageNum":[NSString stringWithFormat:@"%d",pageNum]}];
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"discountArea.action" params:pram success:^(id json) {
            [mbHud hide:YES];
            NSLog(@"折扣专区%@",json);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell reloadDataWithIndexpath:indexPath AndArray:discountArray];
...
-(void)AddCarClick:(UIButton *)btn
{
    if ([isLogin integerValue] == 1) {
        ...
        DiscountModel *model  = discountArray[btn.tag];
...
{
    DiscountModel *model = discountArray[index];
