...
{
    oldPas = noti.userInfo[@"fieldText"];
}
...
-(void)sureBtnClick:(UIButton *)btn
{
    ...
    if ([newfile.text isEqualToString:newfileAgn.text]) {
        if (self.isFrom == 1) {//登录密码
            //重置登陆密码
            [self setLoginPassWithNewPas:newfile.text];
        }
        else
        {
            [self setPayPassWord];
...
#pragma mark 重置登录密码
-(void)setLoginPassWithNewPas:(NSString *)newPas
{
    UITextField *newfile = (UITextField *)[self viewWithTag:1];
    UITextField *newfileAgn = (UITextField *)[self viewWithTag:2];
    ...
    if([oldPas isEqualToString:@"123456"]){
        oldPas = @"-1";
    }
    NSString *mdPasd = [md5_password encryptionPassword:oldPas userTel:user_tel];
...
-(void)setPayPassWord
{
    ...
    if (newfile.text.length == 6) {
        ...
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"paypw.action" params:pram success:^(id json) {
            [Hmud hide:YES];
