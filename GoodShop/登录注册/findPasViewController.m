...
-(void)telSureBtnAction
{
    ...
    if(!isMatch){
        ...
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"user.tel":tel.text}];
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"back.action" params:pram success:^(id json) {
...
-(void)changeNewPasSure
{
    ...
    if ([onePas.text isEqualToString:secPas.text]) {
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"user.tel":userTel,@"pw":onePas.text}];
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"backPassword.action" params:pram success:^(id json) {
            NSDictionary *dic = (NSDictionary *)json;
            if ([[dic valueForKey:@"massage"]intValue]==1) {
                for (int i = 0; i<findWayNameAry.count; i++) {
                    UIView *chooseView =[findWayView viewWithTag:[findWayTagAry[i] integerValue]];
                ...
                [def setValue:onePas.text forKey:@"userPass"];
                MBProgressHUD *mbHud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
                mbHud.mode = MBProgressHUDModeText;
...
-(void)sendEmailAccess
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"changePwdByEmail.action" params:pram success:^(id json) {
        [self dismPopView];
        [mbHud hide:YES];
