...
-(void)layoutSubviews
{
    ...
    [_goodLogo sd_setImageWithURL:[NSURL URLWithString:_secmodel.canpinpic] placeholderImage:[UIImage imageNamed:@"yonghutouxiang"] options:SDWebImageRefreshCached];
    //商品名
    ...
    //原价
    if ([[NSString stringWithFormat:@"%@",_secmodel.yuanjia] isEqualToString:@"0.0"]) {
        _goodOldPrice.alpha = 0;
    }
    else if ([[NSString stringWithFormat:@"%@",_secmodel.yuanjia] isEqualToString:@"0"]) {
        _goodOldPrice.alpha = 0;
    }
    else
    {
    ...
    lineView.frame = CGRectMake(10*MCscale, self.height - 0.5*MCscale, self.contentView.width - 20*MCscale, 0.5);
}
@end
