...
-(void)receiveBtnClick:(UIButton *)button
{
    ...
    Hud.labelText = @"请稍等...";
    [Hud show:YES];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"manageFuliShop.action" params:pram success:^(id json) {
        [Hud hide:YES];
        NSLog(@"---%@",json);
        
        if ([self.receiveBenefitsdelegate respondsToSelector:@selector(reloadDataWithIndex:)]) {
            [self.receiveBenefitsdelegate reloadDataWithIndex:[[json valueForKey:@"message"] integerValue]];
        }
