...
-(void)initSubViews
{
    ...
    NSString *telString = [NSString stringWithFormat:@"%@",user_tel];
...
-(void)layoutSubviews
{
    ...
    //收货人
    _receiver.frame = CGRectMake(label.right+5, label.top,self.width - label.width - 45*MCscale, 20*MCscale);
    ...
    menView.frame = CGRectMake(label.right, line1.bottom+7*MCscale, 80*MCscale, 30*MCscale);
    ...
    sexLabel1.frame = CGRectMake(image1.right+5, image1.top, 60, 20*MCscale);
    //女士
    UIView *womanView = [self viewWithTag:105];
    womanView.frame = CGRectMake(label.right+100*MCscale, line1.bottom+7*MCscale, 80*MCscale, 30*MCscale);
    ...
    sexLabel2.frame = CGRectMake(image2.right+5, image2.top, 60, 20*MCscale);
...
-(void)newAdresSureAction
{
    ...
    if ([self.receiVerTel.text isEqualToString:@""] &&[isLogin integerValue]==1) {
        userTelStr = [NSString stringWithFormat:@"%@",user_tel];
    }
    else
    {
    ...
    {
        BOOL isMatch = [BaseCostomer panduanPhoneNumberWithString:userTelStr];
        if(isMatch){
            ...
            if ([_receiver.text isEqualToString:@""]||[self.addresTextView.text isEqualToString:@"例:北京市朝阳区**小区"]) {
                ...
                NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"dianpuid":_dianpuID,@"address.x":jingdu,@"address.y":weidu,@"address.name":usName,@"address.haoma":userTrueTelStr,@"address.address":self.addresTextView.text,@"address.tel":user_id,@"address.menpai":menpaihao}];
                
                NSLog(@"%@",pram);
                
                [HTTPTool postWithUrlPath:HTTPHEADER AndUrl:@"saveAddresss.action" params:pram success:^(id json) {
                    NSLog(@"添加地址%@",json);
                    [locationDic removeAllObjects];
                    [adresBub hide:YES];
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    {
        if ([textField.text isEqualToString:@""] && ![user_id isEqualToString:userSheBei_id]) {
            textField.placeholder = [NSString stringWithFormat:@"%@",user_tel];
        }
        else
