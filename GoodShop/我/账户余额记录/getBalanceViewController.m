...
-(void)relodData
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findYuE.action" params:pram success:^(id json) {
    ...
    if([button.titleLabel.text isEqualToString:@"请绑定提现到账平台"])
    {
        ...
        if (isSeleted==YES) {
            ...
            if (([textFiledValue floatValue] <=[_tixinJe floatValue]) && [textFiledValue floatValue] >0) {
                ...
                [HTTPTool postWithUrlPath:HTTPHEADER AndUrl:@"yuETiXian4.action" params:pram success:^(id json) {
