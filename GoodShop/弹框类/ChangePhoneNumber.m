...
-(void)ChangePhoneNumberCkick:(UIButton *)btn
{
    if (btn == sendCode) {//验证码
        ...
        BOOL isMatch = [BaseCostomer panduanPhoneNumberWithString:numPh.text];
        if(!isMatch){
            ...
            [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"showCode.action" params:jsonDict success:^(id json) {
    ...
    else if(btn == sureBtn)//确认更换
    {
        ...
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"uqdatephone.action" params:pram success:^(id json) {
            if ([[json valueForKey:@"massage"]integerValue]==4) {
                ...
                [[NSUserDefaults standardUserDefaults]setValue:numPh.text forKey:@"userTel"];
