...
-(void)initNavigation
{
    ...
    [rightButton setImage:[UIImage imageNamed:@"收藏"] forState:UIControlStateNormal];
...
-(void)rightItemClick
{
    if ([isLogin integerValue] == 1) {
        ...
        mbHud.delegate =self;
        [mbHud show:YES];
        NSMutableDictionary *parm = [[NSMutableDictionary alloc]initWithDictionary:@{@"shoucang.userid":user_id,@"shoucang.shoucangtype":@"0",@"shoucang.shopid":@"0",@"shoucang.dianpuid":_dianpuId,}];
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"shoucang.action" params:parm success:^(id json) {
            [mbHud hide:YES];
            NSLog(@"%@",json);
...
-(void)reloadData
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findbyshequdianpuid.action" params:pram success:^(id json) {
        NSLog(@"店铺数据%@",json);
...
-(void)initTableView
{
    ...
    shopMessageBtn  = [UIButton buttonWithType:UIButtonTypeCustom];
    shopMessageBtn.frame = CGRectMake(0, 0, kDeviceWidth/2.0, 38*MCscale);
    shopMessageBtn.backgroundColor = [UIColor clearColor];
    shopMessageBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
    [shopMessageBtn setTitle:@"商家介绍" forState:UIControlStateNormal];
//    [shopMessageBtn setTitleColor:txtColors(25, 182, 133, 1) forState:UIControlStateNormal];
    [shopMessageBtn setTitleColor:textBlackColor forState:UIControlStateNormal];
    [shopMessageBtn setTitleColor:mainColor forState:UIControlStateSelected];
    [shopMessageBtn addTarget:self action:@selector(shopMessageBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    shopMessageBtn.titleLabel.font = [UIFont systemFontOfSize:MLwordFont_3];
    [btnView addSubview:shopMessageBtn];
    shopMessageBtn.selected=YES;
    
    evaluateBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    evaluateBtn.frame = CGRectMake(kDeviceWidth/2.0, 0, kDeviceWidth/2.0, 38);
    evaluateBtn.backgroundColor = [UIColor clearColor];
    [evaluateBtn setTitle:@"用户评价" forState:UIControlStateNormal];
    evaluateBtn.titleLabel.textAlignment = NSTextAlignmentCenter;
    [evaluateBtn addTarget:self action:@selector(shopMessageBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [evaluateBtn setTitleColor:textBlackColor forState:UIControlStateNormal];
    [evaluateBtn setTitleColor:mainColor forState:UIControlStateSelected];
    
    evaluateBtn.titleLabel.font = [UIFont systemFontOfSize:MLwordFont_3];
    [btnView addSubview:evaluateBtn];
    ...
    shopDetailTabel = [[UITableView alloc]initWithFrame:CGRectMake(0, 104*MCscale, kDeviceWidth, kDeviceHeight-104*MCscale) style:UITableViewStyleGrouped];
...
-(void)getEvaluateData
{
    ...
    mbHud.delegate =self;
    [mbHud show:YES];
    NSMutableDictionary *pram = [NSMutableDictionary dictionaryWithDictionary:@{@"dianpuid":_dianpuId,@"leimu":[NSString stringWithFormat:@"%d",isSelected],@"pagemum":[NSString stringWithFormat:@"%d",pageNum]}];
    ...
    else
    {
        ...
        else
        {
            ...
            if (![model.guanjia isEqualToString:@"0"]) {
                ...
                cellHeight = huifuSize.height+cellHeight;
            }
            NSLog(@"%lf",cellHeight);
        }
        return cellHeight;
