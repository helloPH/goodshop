...
#define MBPFont_5 (IPHONE_5 ? 14.f : 16.f)
...
//static const CGFloat kLabelFontSize = MBPFont_5 ;
...
+(MB_INSTANCETYPE)showHUDAddTo:(UIView *)view aboveView:(UIView *)aboveView animated:(BOOL)animated
{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    hud.removeFromSuperViewOnHide = YES;
    [view insertSubview:hud aboveSubview:aboveView];
    [hud show:animated];
    return MB_AUTORELEASE(hud);
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.opacity = 0.8f;
		self.color = nil;
		self.labelFont = [UIFont boldSystemFontOfSize:MBPFont_5];
