...
-(void)reloadMoneys
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"dingdantotalprice.action" params:pram success:^(id json) {
        ...
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"usertel":user_id,@"money":[NSString stringWithFormat:@"%@",moneyModel.totalPrices],@"dianpuid":_dianpuID}];
...
-(void)changeRelodMoey:(NSString *)adresId
{
    ...
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id,@"shdzid":adresId,@"youhuijuan":youh,@"dianpuid":_dianpuID}];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"dingdantotalprice.action" params:pram success:^(id json) {
        NSDictionary *dic = [json valueForKey:@"moneys"];
        for (int j = 0; j<dpuid.count; j++) {
            NSDictionary *dc = [dic valueForKey:dpuid[j]];
...
-(void)relodYouh
{
    orderMoney *model = orderModleAry[orderModleAry.count-1];
    if (!isChooseYh) {
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_tel,@"jine":model.totalPrices,@"dianpuid":_dianpuID}];
...
#pragma mark -- NewAddresDelegate(新增地址)
...
//        [self initDefaulAddressData];
    ...
    {
        [self promptMessageWithString:@"含有活动限制商品,无法保存"];
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        titleArray = @[@"使用优惠",@"现金付款",@"在线支付",@"余额支付",@"消费券"];
    }
...
-(void)btnAction:(UIButton *)btn
{
    if (btn.tag == 1001) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            [changeAddressPopView relodaDataWithDianpuID:_dianpuID AndDefaultAddress:defaultdizId];
...
-(void)messageNetxAction
{
    ...
    NSString *pasdStr = [md5_password encryptionPassword:fild.text userTel:user_tel];
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"user.tel":user_id,@"user.password":pasdStr}];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findbydenglu1.action" params:pram success:^(id json) {
        NSString *message = [NSString stringWithFormat:@"%@",[json valueForKey:@"massages"]];
        if ([message isEqualToString:@"2"]) {
            ...
            [self promptMessageWithString:@"登录密码错误!请重新输入"];
        }
    } failure:^(NSError *error) {
        [self promptMessageWithString:@"网络连接错误17"];
    }];
}
