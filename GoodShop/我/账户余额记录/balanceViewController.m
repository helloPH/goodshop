...
{
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id}];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findbyuseridday.action" params:pram success:^(id json) {
        
        NSArray *ary = [json valueForKey:@"jilushow"];
        if (ary.count > 0) {
            for(NSDictionary *dic in ary){
                blanceModel *model = [[blanceModel alloc]init];
...
-(void)initMask
{
    masView = [[UIView alloc]initWithFrame:CGRectMake(0, 64, kDeviceWidth, kDeviceHeight-64)];
    masView.alpha = 0;
...
-(void)btnAction:(UIButton *)btn
{
    ...
    if (btn.tag == 101) {
        ...
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id}];
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"checkTixianInfo.action" params:pram success:^(id json) {
