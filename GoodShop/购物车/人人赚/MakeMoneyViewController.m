...
-(void)initData
{
    ...
    dbud.labelText = @"请稍等...";
    [dbud show:YES];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"enterEarnMoney.action" params:nil success:^(id json) {
        [dbud hide:YES];
        NSLog(@"人人赚 %@",json);
...
-(void)receiveNewGift
{
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id,@"dianpuid":@"0",@"shequid":@"",@"hongbaotype":@"1"}];
                                   ...
                                   shareOptions:nil
                                         result:^(ShareType type, SSResponseState state, id<ISSPlatformShareInfo> statusInfo, id<ICMErrorInfo> error, BOOL end) {
                                             if (state == SSResponseStateSuccess){
                                                 NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id,@"shequid":@"0",@"dianpuid":user_qiehuandianpu,@"fenxiangstatus":@"1",@"danhao":@"0"}];
                                                 NSLog(@"%@",pram);
                                                 
                                                 [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"fenxiang.action" params:pram success:^(id json) {
