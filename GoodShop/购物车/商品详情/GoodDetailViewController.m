...
{
    [[NSUserDefaults standardUserDefaults]setValue:_dianpuId forKey:@"dianpuqiehuan"];
...
-(void)addToCardData
{
     ...
     *  userid	    		//用户id
     car.dianpuid 	//店铺id
     shangpins		//商品id(拼接)详情，关联.....
     car.xinghao	//型号(拼接)详情，关联.....
     car.yanse		//颜色(拼接)详情，关联.....
     shuliangs		//数量(拼接)详情，关联.....
     （数组长度一致，用0补齐）
...
{
    goodDeailModel *model = goodDataAry[0];
    NSMutableDictionary *parm = [[NSMutableDictionary alloc]initWithDictionary:@{@"shoucang.userid":user_id,@"shoucang.shoucangtype":@"1",@"shoucang.shopid": [NSString stringWithFormat:@"%@",model.shangpinid],@"shoucang.dianpuid":_dianpuId}];
