...
-(void)yuezhifuAction:(NSString *)zhfimm
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findbyuserids.action" params:mimaDic success:^(id json) {
        if ([[json valueForKey:@"massages"]integerValue] ==1) {
            if ([self.paymentPasswordDelegate respondsToSelector:@selector(PaymentSuccessWithIndex:)]) {
                [self.paymentPasswordDelegate PaymentSuccessWithIndex:indexss];
            }
        }
        else{
