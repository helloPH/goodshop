...
+ (void)postWithUrlPath:(NSString *)urlStr AndUrl:(NSString *)url params:(NSMutableDictionary *)params fileArray:(NSArray *)fileArr success:(successBlock)success failure:(failureBlock)failure
{
    // 1.获取AFN的请求管理者
    AFHTTPRequestOperationManager *manger = [AFHTTPRequestOperationManager manager];
    //网络延时设置15秒
...
+ (void)postWithUrlPath:(NSString *)urlStr AndUrl:(NSString *)url params:(NSMutableDictionary *)params image:(UIImage *)image success:(successBlock)success failure:(failureBlock)failure
{
    // 1.获取AFN的请求管理者
    AFHTTPRequestOperationManager *manger = [AFHTTPRequestOperationManager manager];
    //网络延时设置15秒
...
constructingBodyWithBlock:^(id<AFMultipartFormData> formData){
         ...
         NSString *fileName = [NSString stringWithFormat:@"%@file.jpeg",user_id];
