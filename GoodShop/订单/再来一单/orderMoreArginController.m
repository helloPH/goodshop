...
@interface orderMoreArginController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate,PopViewDelegate,UITextViewDelegate,addressDelegate,MBProgressHUDDelegate,UITextFieldDelegate,newAddresPopDelegate,PaymentPasswordViewDelegate,SetPaymentPasswordViewDelegate,LoginPasswordViewDelegate,SelectedTimeViewDelegate,ChangeAddressViewDelegate,OnLinePayViewDelegate>
{
    ...
    ChangeAddressView *changeAddressPopView;//更换地址
...
- (void)viewDidLoad {
    ...
    cellCount = 4;
    lastChoose =10;
    labelTextAlignment = isYouhuiquan  = 0;
    choseYouh = @"0";
    isChooseYh = 0;
    popViewTag = -1;
    isRecharge = 0;
...
-(void)reldData
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"showdingdan.action" params:pram success:^(id json) {
        NSLog(@"-----再来一单%@",json);
        ...
        agnAdresModel *model = [[agnAdresModel alloc]init];
        ...
        for(NSDictionary *dc in ary){
            orderAgnGoodModle *modl = [[orderAgnGoodModle alloc]init];
            [modl setValuesForKeysWithDictionary:dc];
...
-(void)reloadMoneys
{
    ...
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id,@"dindan.danhao":_danhao,@"youhuijuan":youh,@"shdzid":@"-1"}];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"dingdanjine.action" params:pram success:^(id json) {
        NSDictionary *dic = (NSDictionary *)json;
        orderMoney *ormodl = [[orderMoney alloc]init];
        [ormodl setValuesForKeysWithDictionary:dic];
        [orderModleAry addObject:ormodl];
        [self loadFootView];
        [self initTableCellData];
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"usertel":user_id,@"money":[NSString stringWithFormat:@"%@",ormodl.totalPrices],@"dianpuid":dianpuID}];
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findbyyonghulibao.action" params:pram success:^(id json) {
            NSLog(@"礼包 %@",json);
            userLibaoStr = [NSString stringWithFormat:@"%@",[json valueForKey:@"massages"]];
...
-(void)initTableCellData
{
    if(orderGoodDataAry.count>0 && orderModleAry.count>0){
        ...
        for (int i= 0; i<dataArray.count; i++) {
            ...
            NSString *st = [NSString stringWithFormat:@"%@",model.yuyueshuoming];
...
{
    if (!isChooseYh) {
        orderMoney *ormodl = orderModleAry[0];
        //        agnAdresModel *model = dataArray[0];
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_tel,@"jine":ormodl.totalPrices,@"dianpuid":dianpuID}];
        
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"useYouhui.action" params:pram success:^(id json) {
            NSLog(@"---json %@",json);
            if ([[json valueForKey:@"massages"]integerValue]!=1) {
                NSArray *ary = [json valueForKey:@"youhuiquan"];
                for(NSDictionary *dic in ary){
                    youhuiModel *mod = [[youhuiModel alloc]init];
...
-(void)initTableView
{
    orderTableView = [[UITableView alloc]initWithFrame:CGRectMake(0,64, kDeviceWidth, kDeviceHeight-64-49*MCscale) style:UITableViewStyleGrouped];
...
-(void)newAddressView:(NewAddresPopView *)popView Andvalue:(NSInteger)index
{
    ...
    {
        [self promptMessageWithString:@"保存失败"];
    }
}
//增加地址后刷新数据
...
-(void)addNewAddresMoney
{
    ...
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id,@"dindan.danhao":_danhao,@"youhuijuan":youh,@"shdzid":defaultdizId}];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"dingdanjine.action" params:pram success:^(id json) {
        //(@"---json==%@",json);
        NSDictionary *dic = (NSDictionary *)json;
        orderMoney *moneyModel = [[orderMoney alloc]init];
        [moneyModel setValuesForKeysWithDictionary:dic];
...
{
    changeAddressPopView = [[ChangeAddressView alloc]initWithFrame:CGRectMake(kDeviceWidth/20.0, 100*MCscale, kDeviceWidth*9/10.0, 268*MCscale)];
    changeAddressPopView.tag = 107;
    changeAddressPopView.changeDelegate = self;
}
//切换地址获取金额优惠
-(void)changeRelodMoey:(NSString *)adresId
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"dingdanjine.action" params:pram success:^(id json) {
        ...
        [self initTableCellData];
        [self changeFootData];
        NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"usertel":user_id,@"money":[NSString stringWithFormat:@"%@",moneyModel.totalPrices],@"dianpuid":dianpuID}];
        [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findbyyonghulibao.action" params:pram success:^(id json) {
            NSLog(@"礼包 %@",json);
            userLibaoStr = [NSString stringWithFormat:@"%@",[json valueForKey:@"massages"]];
        } failure:^(NSError *error) {
...
-(void)PaymentSuccessWithIndex:(NSInteger)index
{
    ...
    [HTTPTool postWithUrlPath:HTTPHEADER AndUrl:@"saveagain.action" params:ppmdic success:^(id json) {
        ...
        if ([message isEqualToString:@"1"]) {
            ...
            if (index == 2) {
                NSMutableDictionary *pramDic = [[NSMutableDictionary alloc]initWithDictionary:@{@"usertel":user_id}];
                [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"savelibaobaojilu.action" params:pramDic success:^(id json) {
                } failure:^(NSError *error) {
        ...
        [allBub hide:YES];
        [self promptMessageWithString:@"网络连接错误10"];
    }];
}
...
//                    line.backgroundColor = txtColors(231, 232, 234, 0.8);
        ...
        if (indexPath.row < scnum) {
            ...
            [logo sd_setImageWithURL:[NSURL URLWithString:model.shopimg] placeholderImage:[UIImage imageNamed:@"yonghutouxiang"] options:SDWebImageRefreshCached];
...
-(void)btnAction:(UIButton *)btn
{
    if (btn.tag == 1002) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            [changeAddressPopView relodaDataWithDianpuID:dianpuID AndDefaultAddress:defaultdizId];
...
-(void)choosePayAction:(NSInteger)btnTag
{
    ...
    if (btnTag == 4) {
        if ([zhifumima isEqualToString:@"1"]) {
            [self promptMessageWithString:@"还没有支付密码!请先设置一下吧"];
            //未设置支付密码
            [self performSelector:@selector(popPaswardView) withObject:self afterDelay:2.5];
        }
    }else
...
-(void)daofuAction
{
    [HTTPTool postWithUrlPath:HTTPHEADER AndUrl:@"saveagain.action" params:ppmdic success:^(id json) {
        ...
        if ([message isEqualToString:@"1"]) {
            ...
            [allBub hide:YES];
            [self promptMessageWithString:@"下单失败!请稍后尝试"];
        }
    } failure:^(NSError *error) {
        [allBub hide:YES];
        [self promptMessageWithString:@"网络连接错误12"];
    }];
}
...
-(void)goToCheck
{
    ...
    if ([nameL.text isEqualToString:@""] || [telL.text isEqualToString:@""] || [adresL.text isEqualToString:@""]) {
        ...
        NSString *dianpuidStr =[NSString stringWithFormat:@"%@",dianpuID]; //店铺id 拼接
        NSString *bzstr = @"0";
        ...
        if (lastChoosePay == 3||lastChoosePay == 4) {
            if ([zhifumima isEqualToString:@"2"]) {
                ...
                [UIView animateWithDuration:0.3 animations:^{
                    ...
                    [passPopView reloadDataWithIndex:lastChoosePay - 2];
...
-(void)zhifubaoAction
{
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"shequid":@""}];
    NSMutableDictionary *privateDic = [[NSMutableDictionary alloc]init];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"zhifu.action" params:pram success:^(id json) {
        if(json){
        ...
        [allBub hide:YES];
        [self promptMessageWithString:@"网络连接错误13"];
    }];
}
...
-(void)payMoney:(NSMutableDictionary *)dic
{
    ...
    order.productName =[NSString stringWithFormat:@"妙店佳+%@支付",orderid]; //商品标题
...
{
    [self promptMessageWithString:@"暂无更多的充值方法"];
}
//充值成功的通知
-(void)RechargSuccess
{
...
-(void)chooseSeccJump
{
    ...
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"findjinqidingdan.action" params:pram success:^(id json) {
        NSLog(@"-- zhifu2 -- ");
        [allBub hide:YES];
        ...
        detailVC.dingdanId = dnhaol;
        detailVC.isOrderAgn = 0;
        ...
        [allBub hide:YES];
        [self promptMessageWithString:@"网络连接错误15"];
    }];
}
...
-(void)refreshYouhuiMoney
{
    ...
    //    agnAdresModel *model = dataArray[0];
    NSMutableDictionary *pram = [[NSMutableDictionary alloc]initWithDictionary:@{@"userid":user_id,@"shdzid":defaultdizId,@"youhuijuan":youh,@"dianpuid":dianpuID}];
    [HTTPTool getWithUrlPath:HTTPHEADER AndUrl:@"dingdantotalprice.action" params:pram success:^(id json) {
        ...
        for (int j = 0; j<dpuid.count; j++) {
            ...
            orderMoney *ormodl = [[orderMoney alloc]init];
            [ormodl setValuesForKeysWithDictionary:dc];
            [orderModleAry addObject:ormodl];
        }
        orderMoney *moneyModel = [[orderMoney alloc]init];
        [moneyModel setValuesForKeysWithDictionary:dic];
        ...
        [self promptMessageWithString:@"网络连接错误16"];
    }];
}
...
-(void)SetPaymentPasswordSuccessWithIndex:(NSInteger)index
{
    ...
    {
        [self promptMessageWithString:@"修改失败!请稍后重试"];
    }
}
